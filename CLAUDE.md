...
backend/
  src/
    ...
    mxiWriter/                     Phase 2
      ...
      saveStorageState.ts          one-off: `npm run mxi:save-storage-state` — logs in, saves authenticated session to data/mxi-stage-storage-state.json so codegen recordings can start pre-authenticated. Always stage — no --env option (this tool is specifically for building stage codegen recordings, not part of the order-data flow).
